...
var bind_node: Object: set = set_bind_node
...
func set_bind_node(node: Object): bind_node = node; if !node: stop()

func _process(delta: float) -> void:
	if is_playing and (!bind_node or !bind_node is Node or bind_node.is_inside_tree() and bind_node.can_process()): step += delta
